...
class ReCaptchaV2
{
	...
	const CAPTCHA_CHALLANGE_PREFIX='g-recaptcha';
	public function getResponseFieldName(){ return self::CAPTCHA_CHALLANGE_PREFIX.'-response'; }
...
SCRIPT;
        ...
        $responseField  = $this->getResponseFieldName();
        if (!empty($name)) {
        ...
        $return .= <<<HTML
<script type="text/javascript"
   src="{$host}.js{$errorPart}">
</script>
HTML;
        $return .= '<div class="'.self::CAPTCHA_CHALLANGE_PREFIX.'" data-sitekey="'.$this->publicKey.'"></div>';
     ...
     * @throws \ZendService\ReCaptcha\Exception
     */
    protected function post($pResponseDataOrPostData)
    {
		...
		if(is_array($pResponseDataOrPostData)){
			$pResponseDataOrPostData=$pResponseDataOrPostData[$this->getResponseFieldName()];
		}

        /* Fetch an instance of the http client */
        $httpClient = $this->getHttpClient();

        $postParams = array('secret' => $this->privateKey,
                            #'remoteip'   => $this->ip,
                            #'challenge'  => $challengeField,
                            'response'   => $pResponseDataOrPostData);
     ...
     */
    public function verify($pResponseDataOrPostData)
    {
        $response = $this->post($pResponseDataOrPostData);
		#die('@'.__LINE__.': '.__FILE__.'<pre>'.print_r($response->getBody(), true));
