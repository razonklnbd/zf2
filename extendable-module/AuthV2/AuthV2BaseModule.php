...
class AuthV2BaseModule {
    ...
    private $closureForPostLoginRoutine;
    public function setClosureForPostLoginRoutine(\Closure $pClosure){
        $this->closureForPostLoginRoutine=$pClosure;
        return $this;
    }
    private function getClosureForPostLoginRoutine(){
        if(isset($this->closureForPostLoginRoutine)) return $this->closureForPostLoginRoutine;
        return function(\Zend\ServiceManager\ServiceManager $sm){
            /** nothing to execute, please setclosure to run post login routine by setClosureForPostLoginRoutine **/
            ### need to return something
            return function(AuthenticationService $authenticationService){};
        };
    }
...
//    }
    ...
    private $racsDataHandlerClosure;
    public function setRacsDataHandlerClosure(\Closure $racsDataHandlerClosure){
        $this->racsDataHandlerClosure=$racsDataHandlerClosure;
        return $this;
    }
    private function getRacsDataHandlerClosure(){
        if(!isset($this->racsDataHandlerClosure)){
            throw new \Exception('RACS Data Handler Closure NOT Provided!!!');
            /* return function(\Zend\ServiceManager\ServiceManager $sm){
                 return function(\Racs\Interfaces\AuthenticatedUserObjectInterface $loggedInUserObject)use($sm){
                     $companyObject=$loggedInUserObject->getCompany();
                     $racsDataFileLocation=SysHelper::getSysDataPath().'racs-data.json';
                     return new \Racs\AbstractRacsDataHandler($sm->get('Racs\\DbAdapter'), $racsDataFileLocation, $companyObject->getMasterVendorTextId(), $companyObject->getTextId());
                 };
             }; */
        }
        return $this->racsDataHandlerClosure;
    }
    private $racsDesignationTableClosure;
    public function setRacsDesignationTableClosure(\Closure $racsDesignationTableClosure){
        $this->racsDesignationTableClosure=$racsDesignationTableClosure;
        return $this;
    }
    private function getRacsDesignationTableClosure(){
        if(!isset($this->racsDesignationTableClosure)) throw new \Exception('RACS Designation Table Closure NOT Provided!!!');
        return $this->racsDesignationTableClosure;
    }
    ...
    private $additionalRegisteredServices;
    public function registerAdditionalService($index, $closure){
        $this->additionalRegisteredServices[$index]=$closure;
        return $this;
    }
    private function getAdditionalRegisteredServices(){
        if(!isset($this->additionalRegisteredServices)) return array();
        return $this->additionalRegisteredServices;
    }
    public function getServiceConfig(array $pExistingServiceArr=array()){
        ...
        $s2r['AuthV2\\SysPrefix']=function (\Zend\ServiceManager\ServiceManager $sm) use($sysPrefixClsr) {
            #die('sysprefix: '.$sysPrefixClsr($sm).' @'.__LINE__.': '.__FILE__);
            return $sysPrefixClsr($sm); };
        $authV2UserLoggedInHandlerServiceIdx='AuthV2\\UserLoginHandler';
        if(isset($this->usrLoginHndlrSrvcIdx)){
            $origiUserLoggedInHandlerServiceIdx=$this->usrLoginHndlrSrvcIdx;
            $s2r[$origiUserLoggedInHandlerServiceIdx]=function (\Zend\ServiceManager\ServiceManager $sm) {
            ...
            $s2r[$authV2UserLoggedInHandlerServiceIdx]=function (\Zend\ServiceManager\ServiceManager $sm) use($origiUserLoggedInHandlerServiceIdx) {
                return $sm->get($origiUserLoggedInHandlerServiceIdx);
            };
        }else{
            $s2r[$authV2UserLoggedInHandlerServiceIdx]=function (\Zend\ServiceManager\ServiceManager $sm) use($origiUserLoggedInHandlerServiceIdx) {
                if(\AuthV2\Api\AuthV2LoggedInUserHandler::isExists()) return \AuthV2\Api\AuthV2LoggedInUserHandler::getOnce();
                #echo '@'.__LINE__.': '.__FILE__.'<br />';
                return \AuthV2\Api\AuthV2LoggedInUserHandler::getOnce($sm->get('AuthenticationService'));
            };
        }
        ...
        $s2r['AuthV2\\PostLoginRoutine']=$this->getClosureForPostLoginRoutine();
        
        ### RACS
        try {
            $s2r['AuthV2\\RacsDataHandlerClosure']=$this->getRacsDataHandlerClosure();
            $s2r['AuthV2\\RacsDesignationTableClosure']=$this->getRacsDesignationTableClosure();
        } catch (\Exception $e) {
        }

        
        $addtnlSrvcs=$this->getAdditionalRegisteredServices();
        foreach($addtnlSrvcs as $k=>$srvc) $s2r[$k]=$srvc;
        foreach($s2r as $k=>$srvc) $pExistingServiceArr['factories'][$k]=$srvc;
        return $pExistingServiceArr;
    }
    private $loginFormChangeClosure;
    public function setLoginFormChangeClosure(\Closure $loginFormChangeClosure){
	    	$this->loginFormChangeClosure=$loginFormChangeClosure;
	    	return $this;
    }
    private function getLoginFormChangeClosure(){
        if(!isset($this->loginFormChangeClosure)){
            return function (View\Helper\LoginForm $loginForm) {
                return $loginForm;
            };
        }
    	return $this->loginFormChangeClosure;
    }
    public function getViewHelperConfig(array $pExistingServiceArr=array()) {
        $lgnFrmChngClsr=$this->getLoginFormChangeClosure();
        $pExistingServiceArr['factories']['loginForm']=function(\Zend\View\HelperPluginManager $helperPluginManager) use($lgnFrmChngClsr) {
                        $viewHelper = new View\Helper\LoginForm();
                        $viewHelper->setServiceLocator($helperPluginManager->getServiceLocator());
                        return $lgnFrmChngClsr($viewHelper);
                };
        return $pExistingServiceArr;
    }
}
 ...
 * 
    ...
    more options...........
    $this->baseModule->setLoginFormChangeClosure(function(\AuthV2\View\Helper\LoginForm $loginForm){
                $loginForm->setLoginFormChangeClosure(function($lgnForm){
                    if($lgnForm instanceof \AuthV2\Form\LoginForm) $lgnForm->get('loginId')->setLabel('ID');
                    return $lgnForm;
                });
                return $loginForm;
            });
